<div class="modal-dialog modal-lg" role="document">
  ...
  @endphp
    ...
    {!! Form::open(['url' => $url, 'method' => 'PUT', 'id' => 'contact_edit_form', 'enctype' => 'multipart/form-data']) !!}
    ...
    <div class="modal-body">
      ...
      <div class="row">
        ...
        <div class="col-md-3 individual"  @if($contact->contact_type == 'business' || empty($contact->contact_type)) style="display: none;"  @endif>
            ...
            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('pep_type', __('PEP Type') . ':') !!}
                    <div class="input-group">
                        <select name="pep_type" id="pep_type" class="form-control">
                            <option value="" >--Select PEP Type--</option>
                            <option value="Yes" {{($contact->pep_type=='Yes')?'selected':''}}>Yes</option>
                            <option value="No" {{($contact->pep_type=='No')?'selected':''}}>No</option>
                        </select>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('risk_rating', __('Risk Rating') . ':') !!}
                    <div class="input-group">
                        <select name="risk_rating" id="risk_rating" class="form-control">
                            <option value="" >--Select Risk Rating--</option>
                            <option value="High" {{($contact->risk_rating=='High')?'selected':''}}>High</option>
                            <option value="Medium" {{($contact->risk_rating=='Medium')?'selected':''}}>Medium</option>
                            <option value="Low" {{($contact->risk_rating=='Low')?'selected':''}}>Low</option>
                        </select>
                    </div>
                </div>
            </div>
            {{--<div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('passport_picture', __('ID/Passport Picture') . ':') !!}
                    <div class="input-group">
                        <input id="passport_picture" class="form-control" type="file" name="passport_picture">
                        @if(isset($contact) && $contact->passport_picture)
                            <img src="{{ asset($contact->passport_picture) }}" width="100" class="mt-2">
                        @endif
                    </div>
                </div>
            </div>--}}
