...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        ->leftJoin(
            ...
            'transactions as return_sell',
            'ct.transaction_id',
            '=',
            'return_sell.return_parent_id'
        );
        ...
        $register_details = $query->select(
            ...
            DB::raw("SUM(IF(ct.transaction_type = 'sell', ct.amount, 0)) as total_sale"),
            DB::raw("SUM(IF(ct.transaction_type = 'sell', ct.amount, 0)) - SUM(IF(return_sell.type = 'sell_return', return_sell.final_total, 0)) as net_sale"),
            DB::raw("SUM(IF(transaction_type='expense', IF(transaction_type='refund', -1 * amount, amount), 0)) as total_expense"),
            DB::raw("SUM(IF(ct.pay_method='cash', IF(ct.transaction_type='sell', amount, 0), 0)) - SUM(IF(return_sell.type = 'sell_return', return_sell.final_total, 0)) as total_cash"),
            ...
            DB::raw("SUM(IF(return_sell.type='sell_return', return_sell.final_total, 0)) as total_refund"),
